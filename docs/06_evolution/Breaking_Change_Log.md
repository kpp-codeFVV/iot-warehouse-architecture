...
本文件记录后续版本中可能影响兼容性的变更。这里的“破坏性”不是说方案错误，而是指接口、事件、数据结构或部署方式改变后，旧客户端、旧服务或旧数据不能无感兼容，需要提前设计迁移策略。
...
v1.0 到 v1.1 最需要关注 BC-001、BC-002 和 BC-007，因为它们直接影响事件契约、设备身份和设备影子模型。这三项处理不好，会让 QAS-001、QAS-004、QAS-007 和 QAS-008 的验证口径变得不稳定。

v1.1 到 v2.0 最需要关注 BC-003、BC-004、BC-005 和 BC-009，因为它们涉及消息中间件、数据库、安全和部署平台替换。这类变更不能只改代码，还要同步更新 ADR、部署视图、运行手册和验证证据。
