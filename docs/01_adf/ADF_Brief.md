...
方案采用边云协同架构：边缘侧负责设备接入、基础校验、短期缓存和恢复补传；云端负责设备影子、库存状态、告警记录、长期存储和 API 查询。当前可运行原型不接入真实硬件，而是使用模拟设备数据跑通主要链路。

本次作业不做完整 WMS 或 ERP，重点放在架构决策、质量属性场景、验证证据和后续演进之间的对应关系。
