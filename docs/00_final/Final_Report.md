...
| 团队名称与成员 | 团队提交前在最终 PDF 封面填写真实姓名、学号、分工和贡献率 |
| 仓库地址 | 团队提交前填写课程平台或 Git 仓库地址 |
| 提交日期 | 团队提交当天填写 |

本报告整合 `docs/` 目录中的架构驱动因素、架构视图、ADR、评估、验证证据、架构演进和运行手册。报告正文用于答辩和 PDF 提交，详细表格和完整证据保留在各阶段源文档中。

## 摘要

本项目选择《软件体系结构》课程期末大作业题目二，围绕物流仓储场景设计一个 IoT 驱动的智能仓储监控与告警平台。系统面向温湿度传感器、重量传感器、RFID 扫码设备和 AGV 状态设备，支持设备接入、设备影子、库存状态更新、异常告警、自动补货事件和边缘缓存补传。

架构采用边云协同方案：边缘侧负责 MQTT 设备接入、基础校验、短期缓存和恢复补传；云端服务负责设备影子、库存事件、告警记录、长期存储和查询 API。项目实现了 3 个可运行 FastAPI 服务，配合 MQTT Broker、Redis 和 TimescaleDB，通过 Docker Compose 一键启动，并用 PoC、压测、故障注入和追溯矩阵证明关键架构决策。

本项目的核心结论是：在课程原型范围内，边云协同、MQTT 接入、设备影子、事件驱动告警和边缘缓存补传能够形成可运行、可追溯、可验证的架构闭环；在生产化阶段，仍需要继续偿还高可用、安全认证、真实吞吐、数据治理和 OTA 等技术债务。
...
本项目的目标不是实现完整 WMS 或 ERP，而是构建一个能证明关键架构决策的课程原型。原型必须能跑起来，同时文档要解释为什么这样设计、如何验证设计是否成立、未来如何演进。
...
这种设计的主要理由是：全云端方案在网络中断时容易丢失设备数据，完整大规模微服务方案又会超出课程原型范围。边云协同能更直接地响应题目二的设备离线缓存、断点续传和低延迟告警要求。
...
当前本地环境使用 Docker Compose，生产部署视图按照云原生思路描述边缘节点、云端服务、存储层、网络分区和高可用策略。Compose 原型证明服务边界和关键通信链路可运行，生产部署仍需在 Phase 5 中演进为多副本、高可用和安全加固方案。
...
题目二的设备接入场景需要低开销、发布订阅、适合弱网络和大量设备连接。MQTT 相比 HTTP 轮询更适合持续设备上报；相比 AMQP 更轻量；相比 CoAP 在通用 Broker、工具链和工程落地上更容易演示。因此 ADR-001 选择 MQTT 作为设备接入协议，并在 Phase 5 中规划从单节点 Mosquitto 演进到生产级 Broker。
...
轻量 ATAM 评估围绕质量属性效用树展开，重点分析实时告警、设备接入扩展、边缘补传、设备影子、时序数据查询和安全接入。评估结果显示，当前架构对“实时告警”和“边缘补传”的主链路支撑较直接，但对生产级吞吐、安全认证、Broker 高可用、数据库治理和 OTA 的实现仍存在技术债务。
...
这些债务不是文档缺陷，而是课程原型为了控制范围有意接受的简化。Phase 5 已经为每项主要债务安排了偿还版本和触发条件。
...
本地压测用于证明脚本、服务链路和接口可以运行，不代表生产吞吐上限。故障注入覆盖核心服务不可用、云端恢复补传和告警链路验证，目的是说明架构对可靠性风险的处理方式。

### 6.3 质量属性追溯

`QA_Traceability.md` 是答辩核心文档，展示从 QAS 到 ADR、视图和证据的闭环。当前结论包括：
...
v1.0 是课程原型版本，已能通过 Docker Compose 启动完整本地环境，并演示设备接入、设备影子、异常告警、库存事件和边缘补传。它的局限在于：单机单实例、认证简化、Broker 未高可用、数据库未生产化、OTA 未实现。
...
后续演进中重点控制事件契约、设备 ID、消息中间件、数据库 Schema、安全机制、REST API 版本、设备影子模型和 OTA 工作流。每项破坏性变更都需要定义影响对象、迁移策略、回滚窗口和关联 ADR/QAS。
...
团队提交最终 PDF 时，应在封面或本节写清楚每位成员的姓名、学号、负责模块和贡献率。建议按实际工作拆分为：
...
本项目完成了题目二要求的核心架构生命周期：从架构驱动因素识别开始，形成质量属性场景；通过 C4 视图和 ADR 给出架构设计；使用轻量 ATAM 识别风险、敏感点、权衡点和技术债务；通过可运行原型、PoC、压测和故障注入提供验证证据；最后用演进路线图和破坏性变更日志说明系统如何从课程原型走向生产候选架构。

当前架构的优势是边界清晰、链路可运行、决策可追溯、验证有证据。当前架构的局限也明确：本地 Compose、单节点中间件、简化认证和有限压测不能代表生产能力。后续工作应按 Phase 5 路线逐步偿还技术债务，而不是一次性扩大到完整工业级 IoT 平台。
...
| Phase 5 | `docs/06_evolution/Breaking_Change_Log.md` | 破坏性变更日志 |
| Runbook | `docs/07_runbook/Local_Setup.md` | 本地环境启动指南 |
| Runbook | `docs/07_runbook/Demo_Steps.md` | 演示步骤手册 |
